...
var proccessMsgsArr = function (msgs) {
  ...
  for (var $thisMsg in $msgs) {
    ...
    // Handle generic file (pdf, doc, etc)
    if ($msgType === "file") {
      var fileUrl = $msg["msg_body"];
      var fileName = fileUrl.split("/").pop();
      var icon = "";
      if (fileName.match(/\.pdf$/i)) {
        icon = '<i class="fa-solid fa-file-pdf fa-2x"></i>';
      } else if (fileName.match(/\.(jpg|jpeg|png|gif)$/i)) {
        icon = '<i class="fa-solid fa-file-image fa-2x"></i>';
      } else {
        icon = '<i class="fa-solid fa-file fa-2x"></i>';
      }
      $msgContent = "";
      $msgContent +=
        '<a href="' +
...
$(window).on("load", function () {
  ...
  $("body").on("click", ".message-box", function (e) {
    ...
    if (
      $(e.target).hasClass("delete-msg-btn") ||
      $(e.target).closest(".delete-msg-btn").length
    )
      return;
